...
- Access to the GCP project (e.g., `rle-nleuro`)
...
```bash
gcloud projects add-iam-policy-binding "rle-nleuro" \
  --member="serviceAccount:github-actions-rle@rle-nleuro.iam.gserviceaccount.com" \
...
```bash
gcloud projects describe rle-nleuro --format="value(projectNumber)"
```
...
gcloud iam service-accounts add-iam-policy-binding \
  "github-actions-rle@rle-nleuro.iam.gserviceaccount.com" \
  --project="rle-nleuro" \
...
| `GCP_SERVICE_ACCOUNT` | `github-actions-rle@rle-nleuro.iam.gserviceaccount.com` |
...
```bash
python scripts/rasterize_ecosystem_to_cog.py --project rle-nleuro
```
...
- **enables the `storage.googleapis.com` API** and **creates a public-read bucket** `gs://rle-nleuro-rle-cogs` if it does not already exist;
...
```
Caller does not have required permission to use project rle-nleuro.
